...
function BookNow() {
  ...
  const getBus = async () => {
    try {
      ...
      const response = await axiosInstance.post(`${window.location.origin}/api/buses/get-bus-by-id`, {
        _id: params.id,
      });
  ...
  const bookNow = async (transactionId) => {
    try {
      dispatch(ShowLoading());
      const response = await axiosInstance.post(`${window.location.origin}/api/bookings/book-seat`, {
        bus: bus._id,
        seats: selectedSeats,
        transactionId,
      });
        ...
        <Row className="mt-3" gutter={[30, 30]}>
          <Col lg={12} xs={24} sm={24}>
            ...
            <div className="flex flex-col gap-2">
              ...
              <hr />
              {/* Removed StripeCheckout component */}
              <button
